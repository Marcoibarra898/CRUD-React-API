...
export default function ListaCuentas({ onAdd, onEdit }: ListaCuentasProps) {
  ...
  const eliminarCuenta = (id: number) => {
    // Aquí implementarías la llamada a tu API para eliminar
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="py-3 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Saldo</th>
                  <th className="py-3 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Estado</th>
                  <th className="py-3 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Usuario ID</th>
                ...
                {cuentasFiltradas.length > 0 ? (
                  cuentasFiltradas.map(cuenta => (
                    <tr key={cuenta.id} className="hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4 whitespace-nowrap">{cuenta.tipoCuenta}</td>
                      <td className="py-3 px-4 whitespace-nowrap">{cuenta.banco}</td>
                      ...
                      <td className="py-3 px-4 whitespace-nowrap">{cuenta.usuarioId}</td>
                      <td className="py-3 px-4 whitespace-nowrap">
                        <div className="flex space-x-2">
                          <button className="text-indigo-600 hover:text-indigo-800">
                            <Eye size={18} />
                          </button>
                          <button className="text-blue-600 hover:text-blue-800" onClick={() => cuenta.id !== undefined && onEdit(cuenta.id)}>
                            <Edit size={18} />
                          </button>
                          <button 
