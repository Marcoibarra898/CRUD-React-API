...
export default function CuentaForm({ cuentaId, onBack }: CuentaFormProps) {
    ...
    const cargarUsuarios = async () => {
      try {
        ...
        const usuariosData = [
          ...
          { id: 3, nombre: 'Carlos', apellido: 'Rodríguez', email: 'carlos@example.com', telefono: '456789123' }
        ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(esEdicion ? 'Cuenta actualizada con éxito' : 'Cuenta creada con éxito');
      onBack();
    } catch (error) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
              ...
              {errores.numeroCuenta && (
                <p className="mt-1 text-sm text-red-600">{errores.numeroCuenta}</p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {errores.saldo && (
                <p className="mt-1 text-sm text-red-600">{errores.saldo}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errores.usuarioId && (
                <p className="mt-1 text-sm text-red-600">{errores.usuarioId}</p>
              )}
            </div>
